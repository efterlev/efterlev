...
- [x] **No long-lived secrets in any release workflow other than `DOCKERHUB_TOKEN`.** GitHub Container Registry (ghcr.io) uses the per-job `GITHUB_TOKEN` (ephemeral). Docker Hub doesn't support keyless OIDC for image push at the time of review, so a long-lived Personal Access Token is required there. This is documented in `release-container.yml:13` and is a known constraint, not an oversight.
