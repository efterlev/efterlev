...
- 348 tests passing (+ 1 E2E skipped by default without `ANTHROPIC_API_KEY`).
...
- Known documentation-vs-code gaps (secret redaction, retry+fallback, etc.) are enumerated in `LIMITATIONS.md` rather than papered over; docs and code agree on what exists.

**What's next (per v1 locked plan + 2026-04-23 review deferred items):**
- **Highest-leverage v1.x single item: secret redaction before LLM transmission.** Evidence content is sent to the LLM verbatim today — see LIMITATIONS.md + the 2026-04-23 review DECISIONS entry for scope. Gated on implementation care rather than calendar; the aspirational THREAT_MODEL claim was corrected, the implementation is the next v1.x thing.
- Retry + Opus-to-Sonnet fallback on transient errors (brings back `fallback_model` config field).
